...
def mask_wearing(root_dir,output_dir=None,dataset_range=None):
    ...
    paths = list()
    detect_flag = False
    # read mask png images just left example on Github
    ...
    if len_mask == 0:
        ...
        if len(dirs) == 0:
            ...
            if detect_flag is True:
                ...
                # for mouth detection and wear mask on faces
                for path in paths:
                    img = cv2.imread(path)
                    # print("haha"+img)
                    if img is None:
...
if __name__ == "__main__":
    ...
    output_dir = r"E:\PycharmProjects\faceNet_and_Data\CASIA-WebFace\CASIA-WebFace_aligned(mask)"
    dataset_range = None
