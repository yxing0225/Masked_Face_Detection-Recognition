...
def model_restore_from_pb(pb_path, node_dict,GPU_ratio=None):
    ...
    with tf.Graph().as_default():
        ...
        if GPU_ratio is None:
            ...
            # control limit the GPU resource
...
class FaceMaskDetection():
    def __init__(self,pb_path,margin=44,GPU_ratio=0.1):
        ...
        # existing model from pb file
        sess, tf_dict = model_restore_from_pb(pb_path, node_dict,GPU_ratio = GPU_ratio)
        tf_input = tf_dict['input']
        model_shape = tf_input.shape  # [Number,Height,Width,Channel]
    ...
    def generate_anchors(self,feature_map_sizes, anchor_sizes, anchor_ratios, offset=0.5):
        ...
        for idx, feature_size in enumerate(feature_map_sizes):
            ...
            # the first scale----->with different aspect ratios
            for ratio in anchor_ratios[idx][1:]:
                # select the first scale
...
def img_alignment(root_dir,output_dir,margin=44,GPU_ratio = 0.1,img_show=False,dataset_range=None):
    # get the start time and record it
    d_t = time.time()
    # var
    ...
    # get know the average process time of each image
    if quantity != 0:
        d_t = time.time() - d_t
...
if __name__ == "__main__":
    ...
    root_dir = r"E:\PycharmProjects\faceNet_and_Data\CASIA-WebFace\CASIA-WebFace"
    output_dir = r"E:\PycharmProjects\faceNet_and_Data\CASIA-WebFace\CASIA-WebFace_aligned"
    margin = 20
    GPU_ratio = None
    img_show = False
    dataset_range = None
